...
class PoisonedInput:
    ...
    def __xor__(self, other):
        ...
        self.operations.append(lambda x: x ^ other)
        return self
    def __rxor__(self, other):
        return self.__xor__(other)
    def __eq__(self, other):
        self.value = other
        for i in self.operations[::-1]:
            self.value = i(self.value)
        poison_values.append(self.value)
        return True
...
class EmoProgram:
    ...
    def run_program(self):
        ...
        while True:
            ...
            print(f'REGISTERS: {self.R}')
            print(f'PC: {self.PC}')
            print(f'ACC: {self.ACC}')
            print(f'PREVIOUS INSTRUCTION: {self.P[self.PC - 1]}')
            print(f'CURRENT INSTRUCTION: {self.P[self.PC]}')
            ...
            if not self.RUNNING:
                ...
                try:
                    if self.input[0] == 'q':
                        sys.exit()
                    if self.input[0] == 'b':
                        self.breakpoint = int(self.input.split()[1])
                        self.BREAKPOINTS.append(self.breakpoint)
                        print(f'breakpoint set at {self.breakpoint}')
                        continue
                    if self.input[0] == 'r':
                        self.RUNNING = True
                except:
